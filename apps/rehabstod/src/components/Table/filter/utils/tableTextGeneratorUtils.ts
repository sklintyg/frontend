...
export const getEmptyTableText = (user: User, tableName: string) => {
  if (!user) {
    return ''
  }
  return `${isUserDoctor(user) ? 'Du har' : 'Det finns'} inga ${tableName} på ${
    user && user.valdVardenhet ? user.valdVardenhet.namn : ''
  } .`
}
