    ...
    <div>
      ...
      <ul className="ml-5 list-disc">
        <li className="-mb-3 text-2xl" style={{ color: idsGraphColors[4] }}>
          <span className="text-neutral-20 align-middle text-sm">{`Kvinnor (${female.count} st, ${Math.round(female.percentage)} %)`}</span>
        </li>
        <li className="text-2xl" style={{ color: idsGraphColors[3] }}>
          <span className="text-neutral-20 align-middle text-sm">{`Män (${male.count} st, ${Math.round(male.percentage)}%)`}</span>
        </li>
      </ul>
    </div>
  )
