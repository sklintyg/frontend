...
export function PatientTableBody({certificates, isDoctor}: { certificates: PatientSjukfallIntyg[]; isDoctor: boolean }) {
  ...
  return (
    ...
    {sortTableList(certificates, getCertificateColumnData).map(
      (certificate) =>
        columns.length > 0 && (
          <tr key={`${certificate.start}${certificate.slut}`} className={user?.valdVardenhet?.id !== certificate.vardenhetId ? 'italic' : ''}>
            {columns
            .filter(({visible}) => visible)
            .filter(({name}) => !(isDoctor && name === PatientColumn.Läkare))
            .map(({name}) => (
              <PatientTableCellResolver key={name} column={name} certificate={certificate}
                                        belongsToOtherUnit={user?.valdVardenhet?.id !== certificate.vardenhetId} list={certificates} />
            ))}
          </tr>
        )
    )}
