import type { MockUser } from './welcomeReducer'

export const mockUserData: MockUser[] = [
  {
    hsaId: 'TSTNMT2321000156-DRAA',
    forNamn: 'Ajla',
    efterNamn: 'Doktor',
    enhetId: 'TSTNMT2321000156-ALMC',
    legitimeradeYrkesgrupper: ['Läkare'],
  },
  {
    hsaId: 'TSTNMT2321000156-DRAF',
    forNamn: 'Alf',
    efterNamn: 'Doktor',
    enhetId: 'TSTNMT2321000156-ALVC',
    legitimeradeYrkesgrupper: ['Läkare'],
  },
  {
    hsaId: 'TSTNMT2321000156-SKAA',
    forNamn: 'Anna',
    efterNamn: 'Sjuksköterska',
    enhetId: 'TSTNMT2321000156-ALMC',
    legitimeradeYrkesgrupper: ['Sjuksköterska'],
  },
  {
    hsaId: 'TSTNMT2321000156-VAAA',
    forNamn: 'Alva',
    efterNamn: 'Vårdadministratör',
    enhetId: 'TSTNMT2321000156-ALMC',
  },
  {
    hsaId: 'TSTNMT2321000156-DRBE',
    forNamn: 'Beata',
    efterNamn: 'Doktor',
    enhetId: 'TSTNMT2321000156-BEVC',
    legitimeradeYrkesgrupper: ['Läkare'],
  },
  {
    hsaId: '199001092387',
    forNamn: 'Maj',
    efterNamn: 'Pärsson',
    enhetId: '',
    legitimeradeYrkesgrupper: ['Privatläkare, Godkänd'],
  },
  {
    hsaId: '199608112380',
    forNamn: 'Petra',
    efterNamn: 'Privat',
    enhetId: '',
    legitimeradeYrkesgrupper: ['Privatläkare, Saknar konto'],
  },
  {
    hsaId: '191212121212',
    forNamn: 'Tolvan',
    efterNamn: 'Tolvanson',
    enhetId: '',
    legitimeradeYrkesgrupper: ['Saknar allt'],
  },
  {
    hsaId: '197705232382',
    forNamn: 'Frida',
    efterNamn: 'Kranstege',
    enhetId: '',
    legitimeradeYrkesgrupper: ['Privatläkare, Saknar avtal'],
  },
  {
    hsaId: '195212222318',
    forNamn: 'Foris',
    efterNamn: 'Dorisson1',
    enhetId: '',
    legitimeradeYrkesgrupper: ['Saknar legitimation'],
  },
  {
    hsaId: '197309069289',
    forNamn: 'Nina Maria',
    efterNamn: 'Greger',
    enhetId: '',
    legitimeradeYrkesgrupper: ['Naprapat'],
  },
  {
    hsaId: '195711092642',
    forNamn: 'Amanda',
    efterNamn: 'Tigersson',
    enhetId: '',
    legitimeradeYrkesgrupper: ['Privatläkare, återkallad'],
  },
  {
    hsaId: 'TSTNMT2321000156-VABE',
    forNamn: 'Bent',
    efterNamn: 'Vårdadministratör',
    enhetId: 'TSTNMT2321000156-BEMC',
  },
  {
    hsaId: 'TSTNMT2321000156-BMBL',
    forNamn: 'Bertil',
    efterNamn: 'Barnmorska',
    enhetId: 'TSTNMT2321000156-ALMC',
    legitimeradeYrkesgrupper: ['Barnmorska'],
  },
  {
    hsaId: 'TSTNMT2321000156-1079',
    forNamn: 'Arnold',
    efterNamn: 'Johansson',
    enhetId: 'TSTNMT2321000156-1077',
    legitimeradeYrkesgrupper: ['Läkare'],
  },
  {
    hsaId: 'TSTNMT2321000156-10CD',
    forNamn: 'Annika',
    efterNamn: 'Larsson',
    enhetId: 'TSTNMT2321000156-1077',
  },
  {
    hsaId: 'TSTNMT2321000156-105T',
    forNamn: 'Ingrid',
    efterNamn: 'Nilsson Olsson',
    enhetId: 'TSTNMT2321000156-105P',
    legitimeradeYrkesgrupper: ['Läkare'],
  },
]

export const mockUserDataSit1: MockUser[] = [
  {
    hsaId: 'TSTNMT2321000156-10M6',
    forNamn: 'Åsa',
    efterNamn: 'Gustafsson',
    enhetId: 'TSTNMT2321000156-10LR',
    legitimeradeYrkesgrupper: ['Läkare'],
  },
  {
    hsaId: 'TSTNMT2321000156-10M6',
    forNamn: 'Åsa',
    efterNamn: 'Gustafsson',
    enhetId: 'TSTNMT2321000156-10LT',
    legitimeradeYrkesgrupper: ['Läkare'],
  },
  {
    hsaId: 'TSTNMT2321000156-10M6',
    forNamn: 'Åsa',
    efterNamn: 'Gustafsson',
    enhetId: 'TSTNMT2321000156-10MS',
    legitimeradeYrkesgrupper: ['Läkare'],
  },
  {
    hsaId: 'TSTNMT2321000156-10LZ',
    forNamn: 'Per',
    efterNamn: 'Johansson',
    enhetId: 'TSTNMT2321000156-10LR',
    legitimeradeYrkesgrupper: ['Läkare'],
  },
  {
    hsaId: 'TSTNMT2321000156-10LT',
    forNamn: 'Per',
    efterNamn: 'Johansson',
    enhetId: 'TSTNMT2321000156-10LR',
    legitimeradeYrkesgrupper: ['Läkare'],
  },
  {
    hsaId: 'TSTNMT2321000156-10M0',
    forNamn: 'Johanna',
    efterNamn: 'Nilsson',
    enhetId: 'TSTNMT2321000156-10LR',
  },
  {
    hsaId: 'TSTNMT2321000156-10M0',
    forNamn: 'Johanna',
    efterNamn: 'Nilsson',
    enhetId: 'TSTNMT2321000156-10LT',
  },
]
