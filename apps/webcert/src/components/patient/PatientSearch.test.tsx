...
describe('PatientSearch', () => {
  ...
  it('should not route if patient is not set', async () => {
    renderComponent()
    await act(() => userEvent.type(screen.getByRole('textbox'), '191212121212'))
    await act(() => userEvent.click(screen.getByText('Fortsätt')))
    act(() => store.dispatch(setPatient(undefined)))
  ...
  it('should route if patient is set', async () => {
    renderComponent()
    await act(() => userEvent.type(screen.getByRole('textbox'), '191212121212'))
    act(() => store.dispatch(setPatient(fakePatient())))
    await act(() => userEvent.click(screen.getByText('Fortsätt')))
  ...
  describe('Input', () => {
    it('should allow user to type patient id', async () => {
      ...
      await act(() => userEvent.type(input, '191212'))
    ...
    it('should add dash when user types patient id', async () => {
      renderComponent()
      const input = screen.getByRole('textbox')
      await act(() => userEvent.type(input, '191212121212'))
  ...
  describe('Validation', () => {
    it('should not validate patient id if focus is on the input', async () => {
      renderComponent()
      const input = screen.getByRole('textbox')
      await act(() => userEvent.type(input, '19121'))
    ...
    it('should validate patient id if focus is not on the input', async () => {
      ...
      const input = screen.getByRole('textbox')
      await act(() => userEvent.type(input, '19121'))
      await act(() => userEvent.click(screen.getByRole('heading')))
    ...
    it('should remove validation error when input gets focus', async () => {
      ...
      await act(() => userEvent.type(input, '19121'))
      await act(() => userEvent.click(screen.getByRole('heading')))
      await act(() => userEvent.type(input, '12'))
      expect(screen.queryByText(EXPECTED_VALIDATION_TEXT)).not.toBeInTheDocument()
    })
    ...
    it('should show validation error without blur if wrong id with 12 numbers', async () => {
      renderComponent()
      const input = screen.getByRole('textbox')
      await act(() => userEvent.type(input, '191212121213'))
