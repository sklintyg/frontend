...
describe('Available for patient status', () => {
  ...
  it('should show modal header', async () => {
    renderComponent(true)
    await act(() => userEvent.click(screen.getByText('Intyget är tillgängligt för patienten')))
  ...
  it('should close modal when pressing close button', async () => {
    renderComponent(true)
    await act(() => userEvent.click(screen.getByText('Intyget är tillgängligt för patienten')))
    await act(() => userEvent.click(screen.getByRole('button', { name: 'Stäng' })))
  ...
  it('shall render specific text in modal if certificate is lisjp', async () => {
    ...
    await act(() => userEvent.click(screen.getByText(/intyget är tillgängligt för patienten/i)))
    expect(
