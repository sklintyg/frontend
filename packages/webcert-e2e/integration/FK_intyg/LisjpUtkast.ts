...
describe('LISJP-utkast tomt', { tags: '@react' }, () => {
  before(() => {
  ...
  context('Användare har möjlighet att uföra följande med ett tomt utkast ', () => {
    ...
    describe('Funktioner på ett tomt LISJP utkast', { tags: '@react' }, () => {
      ...
      it('Det är möjligt att radera ett ifyllt LISJP', () => {
        cy.loggaInVårdpersonalIntegrerat(this.vårdpersonal, this.vårdenhet, this.utkastId)
        const önskadUrl = `/certificate/${this.utkastId}`
        cy.visit(önskadUrl)
        intyg.raderaUtkast()
      ...
      it('Det går inte att signera ett LISJP utkast som inte innehåller alla obligatoriska fält', () => {
        cy.loggaInVårdpersonalIntegrerat(this.vårdpersonal, this.vårdenhet, this.utkastId)
