...
describe('AG7804-intyg låst utkast', { tags: '@react' }, () => {
  before(() => {
  ...
  context('Användare har möjlighet att uföra följande med låst LISJP Utkast ', () => {
    ...
    describe('Funktioner på ett låst AG7804 utkast', { tags: '@react' }, () => {
      it('Skriva ut ett låst AG7804 utkast', () => {
      ...
      it('Makulerar ett låst AG7804 utkast', () => {
        cy.loggaInVårdpersonalIntegrerat(this.vårdpersonal, this.vårdenhet, this.utkastId)
        const önskadUrl = `/certificate/${this.utkastId}`
        cy.visit(önskadUrl)
      ...
      it('Kopiera ett låst AG7804 utkast så att det går att signera', () => {
        cy.loggaInVårdpersonalIntegrerat(this.vårdpersonal, this.vårdenhet, this.utkastId)
        const önskadUrl = `/certificate/${this.utkastId}`
        cy.visit(önskadUrl)
        intyg.kopieraUtkast()
      ...
      it('Ett AG7804  låst utkast ska  inte kunna editeras', () => {
        cy.loggaInVårdpersonalIntegrerat(this.vårdpersonal, this.vårdenhet, this.utkastId)
        const önskadUrl = `/certificate/${this.utkastId}`
        cy.visit(önskadUrl)
