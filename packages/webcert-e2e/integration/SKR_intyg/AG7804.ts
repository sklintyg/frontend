...
describe('AG7804-intyg tomt', { tags: '@react' }, () => {
  before(() => {
  ...
  context('Användare har möjlighet att uföra följande med ett tomt utkast ', () => {
    ...
    describe('Funktioner på ett tomt AG7804 utkast', { tags: '@react' }, () => {
      it('Ett icke ifylld AG7804 går ej att signera och skicka till FK', () => {
      ...
      it('Det är möjligt att raderar ett icke ifyllt AG7804', () => {
        cy.loggaInVårdpersonalIntegrerat(this.vårdpersonal, this.vårdenhet, this.utkastId)
      ...
      it('Skriva ut ett icke ifyllt AG7804', () => {
        cy.loggaInVårdpersonalIntegrerat(this.vårdpersonal, this.vårdenhet, this.utkastId)
        const önskadUrl = `/certificate/${this.utkastId}`
        cy.visit(önskadUrl)
