...
describe('LISJP-intyg tomt', { tags: '@angular' }, () => {
  ...
  context('Användare har möjlighet att uföra följande med ett tomt utkast ', () => {
    ...
    describe('Funktioner då man loggar in på ett utkast från annan vårdgivare med sammanhållenjournal', { tags: '@angular' }, () => {
      it('Ett LISJP utkast ska  inte kunna signeras från en annan vårdgivare', () => {
      ...
      it('Ett LISJP utkast ska  inte kunna raderas från en annan vårdgivare', () => {
        cy.loggaInVårdpersonalIntegrerat(this.vårdpersonalNR1, this.vårdenhetNR1, this.utkastId, true)
        const önskadUrl = `/visa/intyg/${this.utkastId}?enhet=${this.vårdenhetNR1.id}&sjf=true`
        cy.visit(önskadUrl)
        cy.contains('Signera intyget').should('not.exist')
      ...
      it('Ett LISJP utkast ska  inte kunna förnyas från en annan vårdgivare', () => {
        cy.loggaInVårdpersonalIntegrerat(this.vårdpersonalNR1, this.vårdenhetNR1, this.utkastId, true)
        const önskadUrl = `/visa/intyg/${this.utkastId}?enhet=${this.vårdenhetNR1.id}&sjf=true`
        cy.visit(önskadUrl)
        cy.contains('Signera intyget').should('not.exist')
      ...
      it('Ett LISJP utkast ska  inte kunna skrivas ut från en annan vårdgivare', () => {
        cy.loggaInVårdpersonalIntegrerat(this.vårdpersonalNR1, this.vårdenhetNR1, this.utkastId, true)
        const önskadUrl = `/visa/intyg/${this.utkastId}?enhet=${this.vårdenhetNR1.id}&sjf=true`
        cy.visit(önskadUrl)
      ...
      it('Ett LISJP utkast ska  inte kunna editeras från en annan vårdgivare', () => {
        cy.loggaInVårdpersonalIntegrerat(this.vårdpersonalNR1, this.vårdenhetNR1, this.utkastId, true)
        const önskadUrl = `/visa/intyg/${this.utkastId}?enhet=${this.vårdenhetNR1.id}&sjf=true`
        cy.visit(önskadUrl)
