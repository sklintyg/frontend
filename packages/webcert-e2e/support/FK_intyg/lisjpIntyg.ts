  ...
  if (diagnos.rad1) {
    cy.get('#diagnoseCode-0')
      ...
      .within(($diagnoskodrad) => {
        ...
        cy.wrap($diagnoskodrad)
          .contains(diagnos.rad1.text)
          .click()
      })
    cy.get('#diagnoseDescription-0')
      .invoke('val')
      .should('contain', diagnos.rad1.text)
  }

  if (diagnos.rad2) {
    diagnos.rad1.should('exist')
    cy.get('#diagnoseCode-1')
      .parent()
      .within(($diagnoskodrad) => {
        cy.get('#diagnoseCode-1').type(diagnos.rad2.kod)
        cy.wrap($diagnoskodrad)
          .contains(diagnos.rad2.text)
          .click()
      })
    cy.get('#diagnoseDescription-1')
      .invoke('val')
      .should('contain', diagnos.rad2.text)
  }

  if (diagnos.rad3) {
    diagnos.rad2.should('exist')
    cy.get('#diagnoseCode-2')
      .parent()
      .within(($diagnoskodrad) => {
        cy.get('#diagnoseCode-2').type(diagnos.rad3.kod)
        cy.wrap($diagnoskodrad)
          .contains(diagnos.rad3.text)
          .click()
      })
    cy.get('#diagnoseDescription-2')
      .invoke('val')
      .should('contain', diagnos.rad3.text)
  }
}
...
export function sektionBedömning(bedömning): void {
  ...
  const idagPlus1 = Cypress.moment()
    .add(1, 'days')
    .format('YYYY-MM-DD') // 25%  sjukskrivning start
  const idagPlus11 = Cypress.moment()
    .add(11, 'days')
    .format('YYYY-MM-DD') // 25%  sjukskrivning slut
  const idagPlus12 = Cypress.moment()
    .add(12, 'days')
    .format('YYYY-MM-DD') // 50%  sjukskrivning start
  const idagPlus19 = Cypress.moment()
    .add(19, 'days')
    .format('YYYY-MM-DD') // 50%  sjukskrivning slut
  const idagPlus20 = Cypress.moment()
    .add(20, 'days')
    .format('YYYY-MM-DD') // 75%  sjukskrivning start
  const idagPlus28 = Cypress.moment()
    .add(28, 'days')
    .format('YYYY-MM-DD') // 75%  sjukskrivning slut
  const idagPlus29 = Cypress.moment()
    .add(29, 'days')
    .format('YYYY-MM-DD') // 100% sjukskrivning start
  const idagPlus41 = Cypress.moment()
    .add(41, 'days')
    .format('YYYY-MM-DD') // 100% sjukskrivning slut

  const nedsättningArbetsförmåga = bedömning.minBedömningAvPatientensNedsättningAvArbetsförmågan
  nedsättningArbetsförmåga.should('exist')
  if (nedsättningArbetsförmåga.enFjärdedel) {
    cy.get('#sjukskrivningar-EN_FJARDEDEL-from').type(idagPlus1)
    cy.get('#sjukskrivningar-EN_FJARDEDEL-tom')
      .type(idagPlus11)
      .type('{enter}')
  }
  ...
  if (nedsättningArbetsförmåga.hälften) {
    ...
    cy.get('#sjukskrivningar-HALFTEN-tom')
      .type(idagPlus19)
      .type('{enter}')
  }
  ...
  if (nedsättningArbetsförmåga.hel) {
    ...
    cy.get('#sjukskrivningar-HELT_NEDSATT-tom')
      .type(idagPlus41)
      .type('{enter}')
  }
  ...
  if (bedömning.förläggaArbetstidOlika.ja) {
    ...
    cy.get('#arbetstidsforlaggningMotivering').type(bedömning.förläggaArbetstidOlika.arbetstidsförläggningstext)
  } else {
