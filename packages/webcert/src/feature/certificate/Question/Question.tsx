...
const Question: React.FC<QuestionProps> = ({ id, className }) => {
  ...
  const getQuestionComponent = (config: CertificateDataConfig, displayMandatory: boolean, readOnly: boolean) => {
    ...
    if (disabled) {
      return (
        <QuestionHeading readOnly={question.readOnly} id={question.id} hideLabel={hideLabel} question={question} {...question.config} />
      )
    }
    ...
    return (
      <>
        ...
        {<QuestionHeading question={question} readOnly={question.readOnly} id={question.id} hideLabel={hideLabel} {...question.config} />}
      </>
    )
  }
