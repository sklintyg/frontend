...
import { useDeleteQuestionMutation, useSendQuestionMutation, useUpdateQuestionMutation } from '../../store/api'
...
const QuestionForm: React.FC<Props> = ({ questionDraft }) => {
  const [updateQuestion, { isLoading, isUninitialized, isSuccess }] = useUpdateQuestionMutation()
  const [sendQuestion] = useSendQuestionMutation()
  const [deleteQuestion] = useDeleteQuestionMutation()
  const [showValidationMessages, setShowValidationMessages] = useState(false)
  const [message, setMessage] = useState(questionDraft.message)
  const isMissingType = questionDraft.type === QuestionType.MISSING
  const isMissingMessage = questionDraft.message.length === 0
  const isFormEmpty = isMissingType && isMissingMessage
  const isDirty = message !== questionDraft.message
  ...
  const onDropdownChange = (event: ChangeEvent<HTMLSelectElement>) => {
    ...
    updateQuestion({ ...questionDraft, type: event.currentTarget.value as QuestionType })
  }

  const debounceUpdateQuestionDraft = useRef(
    _.debounce((questionDraft: Question, value: string) => {
      updateQuestion({ ...questionDraft, message: value })
    }, 1000)
  ).current

  const onTextAreaChange = (event: ChangeEvent<HTMLTextAreaElement>) => {
    setShowValidationMessages(false)
    debounceUpdateQuestionDraft(questionDraft, event.currentTarget.value)
    setMessage(event.currentTarget.value)
  }

  const handleSendQuestion = () => {
    if (isMissingMessage || isMissingType) {
      setShowValidationMessages(true)
    } else {
      sendQuestion(questionDraft)
    }
  }

  const handleDeleteQuestion = () => {
    questionDraft.id && deleteQuestion(questionDraft.id)
  }
        ...
        <Wrapper>
          ...
          <QuestionFormFooter>
            <div className="ic-forms__group ic-button-group iu-my-400">
              <CustomButton disabled={isFormEmpty || isDirty} buttonStyle="primary" onClick={handleSendQuestion} text="Skicka" />
              <ButtonWithConfirmModal
